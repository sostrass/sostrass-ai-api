...
async def lifespan(app: FastAPI):
    try:
        ...
        pass
    print("[precifica] backend v6.7.2 — boot OK · KPIs de reputação (consumo API + SLA) · tabela avaliacao_cache garantida · leitura do banco + varredura de fundo", flush=True)
    run_migrations()
...
@app.get("/api/shopee/avaliacoes/_diag_api")
def aval_diag_api(item_id: str = "", user: User = Depends(auth.get_current_user)):
    """MEDE o que a API da Shopee realmente devolve: produtos por status, um item
    paginado até o fim e o teto de idade das avaliações alcançáveis."""
    from . import shopee_avaliacoes as av
    try:
        return av.diagnostico_api(user.id, item_id or None)
    except Exception as e:  # noqa: BLE001
        import traceback
        return {"erro": f"{type(e).__name__}: {str(e)[:220]}", "trace": traceback.format_exc()[-400:]}
...
@app.get("/api/shopee/consumo")
def shopee_consumo(dias: int = 14, user: User = Depends(auth.get_current_user)):
    """Consumo da API Shopee: chamadas por endpoint hoje, série de N dias e total do mês.
    Contador em memória (observ) — hoje é exato; a série é desde o último boot."""
    from . import observ
    return observ.consumo(user.id, dias=min(max(dias, 1), 30))


@app.get("/api/shopee/avaliacoes/sla")
def aval_sla(user: User = Depends(auth.get_current_user)):
    """SLA de resposta às avaliações (mediana, P90, faixas) a partir do cache."""
    from . import shopee_avaliacoes as av
    db = SessionLocal()
    try:
        return av.sla_resposta(db, user.id)
    finally:
        db.close()
...
def versao_backend():
    ...
    return {"backend": "v6.7.2", "arquitetura": "banco+varredura", "ts": _time.time()}
